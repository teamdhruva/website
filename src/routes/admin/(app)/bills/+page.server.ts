...
export const load: PageServerLoad = async ({ platform, cookies }) => {
    ...
    if (isAuthorized(user, TREASURER)) {
        const bills = (await platform!.env.D1.prepare('SELECT bills.*, users.name FROM bills JOIN users ON bills.created_by = users.name ORDER BY bills.created_at DESC').all()).results;
        return {
            user,
            bills,
        }
